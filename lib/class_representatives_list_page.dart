...
class DetailedClassRepresentativesPage extends StatefulWidget {
  ...
  @override
  State<DetailedClassRepresentativesPage> createState() =>
      _DetailedClassRepresentativesPageState();
}

class _DetailedClassRepresentativesPageState
    extends State<DetailedClassRepresentativesPage> {
  ...
  void initState() {
    ...
    _allClassReps = List<Map<String, dynamic>>.from(
  ...
  Future<void> _exportToPdf() async {
    ...
    final rows =
        _filteredClassReps.map((classRep) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton(onPressed: _loadClassReps, child: const Text('Reload')),
        ],
      ),
      body: Center(
  ...
  void _confirmDeleteClassRep(
    BuildContext context,
    Map<String, dynamic> classRep,
  ) {
    final name = classRep['name']?.toString() ?? 'Unknown';
    final repId =
        classRep['class_representative_id']?.toString() ??
    ...
    showDialog(
      ...
      builder:
          (dialogContext) => AlertDialog(
            ...
            content: Text(
              'Are you sure you want to delete $name ($repId)?\n\nThis action cannot be undone.',
            ),
            actions: [
              TextButton(
  ...
  Future<void> _deleteClassRep(
    ...
    try {
      ...
      final indexToRemove = _allClassReps.indexWhere(
        (r) => r['class_representative_id'] == repId || r['id'] == repId,
      );
      if (indexToRemove != -1) {
        ...
        AppNotifier.showSuccess(
          context,
          'Class representative removed from this list',
        );
      }
    } catch (e) {
      if (mounted) {
