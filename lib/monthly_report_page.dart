...
class _MonthlyReportPageState extends State<MonthlyReportPage> {
  ...
  Future<void> _loadReport() async {
    ...
    for (final base in _apiCandidates) {
      try {
        final uri = Uri.parse(
          '$base/reports/monthly-report?month=$_selectedMonth&year=$_selectedYear',
        );
  ...
  String _reportFileName() =>
      'Monthly_Report_${_reportData!['report_period']['month_name']}_${_reportData!['report_period']['year']}.pdf';
  ...
  Future<Uint8List> _buildReportPdfBytes() async {
    ...
    final peakEvents =
        (_reportData!['peak_usage_analysis']?['top_peak_events'] as List?) ??
        [];
        ...
        children: [
          ...
          pw.TableRow(
            children: [
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('Total Energy'),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text(stats['total_energy'].toStringAsFixed(2)),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('kWh'),
              ...
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('Aggregated for period'),
              ),
            ],
          ),
          pw.TableRow(
            children: [
              ...
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text(stats['avg_power'].toStringAsFixed(2)),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('W'),
              ...
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('Mean across all sensors'),
              ),
            ],
          ),
          pw.TableRow(
            children: [
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('Peak Power'),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text(stats['peak_power'].toStringAsFixed(2)),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('W'),
              ...
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('Highest recorded'),
              ),
            ],
          ),
          pw.TableRow(
            children: [
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('Active Sensors'),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('${stats['active_sensors']}'),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('count'),
              ...
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('Reporting during period'),
              ),
            ],
          ),
          pw.TableRow(
            children: [
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('Power Factor'),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text(stats['avg_power_factor'].toStringAsFixed(3)),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('pf'),
              ...
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('Average efficiency'),
              ),
            ],
          ),
        ],
    ...
    pw.Widget _peakTable() {
      if (peakEvents.isEmpty) {
        return pw.Text(
          'No peak events recorded in this period.',
              ...
              .map(
                (e) => pw.TableRow(
                  children: [
                    pw.Padding(
                      padding: const pw.EdgeInsets.all(8),
                      child: pw.Text('${e['device_id']}'),
                    ),
                    pw.Padding(
                      padding: const pw.EdgeInsets.all(8),
                      child: pw.Text(e['power'].toStringAsFixed(2)),
                    ),
                    pw.Padding(
                      padding: const pw.EdgeInsets.all(8),
                      child: pw.Text('Peak incident'),
                    ),
                  ],
                ),
              ),
    ...
    pw.Widget _recommendationsList() {
      if (recommendations.isEmpty) {
        return pw.Text(
          'No recommendations recorded for this period.',
          style: const pw.TextStyle(color: PdfColors.grey700),
        );
      }
      return pw.Column(
        children:
            recommendations.take(6).map((rec) {
              final color =
                  rec['priority'] == 'high'
                      ? PdfColors.red700
                      : rec['priority'] == 'medium'
        ...
        children: [
          ...
          pw.TableRow(
            children: [
              ...
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('${sensorStatus['active_sensors'] ?? '-'}'),
              ),
            ],
          ),
          pw.TableRow(
            children: [
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('Inactive Sensors'),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('${sensorStatus['inactive_sensors'] ?? '-'}'),
              ),
            ],
          ),
          pw.TableRow(
            children: [
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('Total Readings'),
              ),
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
                child: pw.Text('${stats['total_readings']}'),
              ),
            ],
          ),
        ],
        ...
        children: [
          ...
          pw.Row(
            ...
            children: [
              _buildPdfStatItem(
                'Avg Voltage',
                '${stats['avg_voltage'].toStringAsFixed(2)} V',
              ),
              _buildPdfStatItem(
                'Power Factor',
              ...
              (event) => pw.Container(
                ...
                child: pw.Row(
                  ...
                  children: [
                    ...
                    pw.Text(
                      '${event['power'].toStringAsFixed(2)} W',
                      style: pw.TextStyle(
                        fontSize: 9,
                        fontWeight: pw.FontWeight.bold,
                      ),
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          TextButton(onPressed: _loadReport, child: const Text('Reload')),
        ],
      ),
  ...
  Widget _buildDailyTrendsChart() {
    ...
    final spots =
        trends.asMap().entries.map((entry) {
          return FlSpot(
            entry.key.toDouble(),
            entry.value['daily_energy'].toDouble(),
          );
        }).toList();
                    ...
                    departments.map((dept) {
                      return DataRow(
                        cells: [
                          ...
                          DataCell(
                            Text(dept['total_energy'].toStringAsFixed(2)),
                          ),
  ...
  Widget _buildHourlyPatternChart() {
    final hourlyPattern =
        _reportData!['peak_usage_analysis']['hourly_pattern'] as List;
            ...
            SizedBox(
              ...
              child: BarChart(
                BarChartData(
                  ...
                  titlesData: FlTitlesData(
                    ...
                    bottomTitles: AxisTitles(
                      sideTitles: SideTitles(
                        ...
                        getTitlesWidget: (value, meta) {
                          if (value.toInt() >= 0 &&
                              value.toInt() < hourlyPattern.length) {
                            return Text(
                              '${hourlyPattern[value.toInt()]['hour']}h',
                  ...
                  barGroups:
                      hourlyPattern.asMap().entries.map((entry) {
