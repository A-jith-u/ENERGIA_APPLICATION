...
class ComputerScienceClassroomsPage extends StatelessWidget {
  const ComputerScienceClassroomsPage({super.key});

  // Sample data for CS Classrooms (now simplified to total usage)
  final List<Map<String, dynamic>> classrooms = const [
    {'name': 'CS-202', 'status': 'Active', 'usage_kw': 3.1, 'color': Colors.green},
    {'name': 'CS-404', 'status': 'Alert', 'usage_kw': 5.2, 'color': Colors.red},
    {'name': 'CS-Lab 1', 'status': 'Offline', 'usage_kw': 0.0, 'color': Colors.grey},
    {'name': 'CS-Seminar Hall', 'status': 'Optimal', 'usage_kw': 1.5, 'color': Colors.blue},
    {'name': 'CS-301', 'status': 'Moderate', 'usage_kw': 4.5, 'color': Colors.orange},
  ];
...
class _ClassroomCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: InkWell(
        onTap: () {
          // ACTION: Navigate to the corresponding CR Dashboard page
          Navigator.push(
            context,
            MaterialPageRoute(
              // Instantiate the CR DashboardPage
